...
export default function EstoqueClient({ estoqueInicial, medicamentosExistentes }: EstoqueClientProps) {
    ...
    const filteredEstoque = estoqueInicial.filter(item => {
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="bg-[#003967] text-white text-xs uppercase font-semibold">
                                <tr>
                                    <th className="px-6 py-4 rounded-tl-lg">Medicamento / Forma</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {filteredEstoque.length > 0 ? (
                                    filteredEstoque.map((item) => {
                                        ...
                                        const valStatus = getValidadeStatus(item.data_de_validade);

                                        return (
                                            <tr key={item.id} className="hover:bg-blue-50/50 transition-colors">
                                                <td className="px-6 py-4">
                                                    <div className="flex flex-col">
                                                        <span className="font-bold text-[#003967] text-base">
                                                            {item.medicamento?.nome || "Medicamento Desconhecido"}
                                                        </span>
                                                        <span className="text-gray-500 font-medium">
                                                            {item.medicamento?.unidade_medida || "-"}
                                                        </span>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <div className="flex flex-col items-center gap-1">
                                                        <span className="font-semibold text-gray-800">
                                                            {item.data_de_validade ? new Date(item.data_de_validade).toLocaleDateString('pt-BR', { timeZone: 'UTC' }) : "-"}
                                                        </span>
