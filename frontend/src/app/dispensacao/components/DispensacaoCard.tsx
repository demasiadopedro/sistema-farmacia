...
import { Pill, ClipboardList, Check, X } from "lucide-react";
...
export function DispensacaoCard({
  prescricao, dispensacao, setDispensacao,
  usoContinuo, buscaPaciente, loading,
  handleDispensacaoSubmit, setStep,
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">
                {prescricao.viaAdministracao} · {prescricao.quantidade} unidades ·{" "}
          ...
          <form onSubmit={handleDispensacaoSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-3 rounded-lg border border-gray-200 px-4 py-3">
              <div>
                ...
                <p className="text-xs text-gray-500">Informe se cada avaliação foi realizada durante a dispensação.</p>
              </div>
              {([
                ...
                <div key={campo} className="flex items-center justify-between gap-4 text-sm text-gray-700">
                  ...
                  <button
                    type="button"
                    aria-label={`${label}: ${dispensacao[campo] ? "sim" : "não"}`}
                    aria-pressed={dispensacao[campo]}
                    onClick={() => setDispensacao((d) => ({ ...d, [campo]: !d[campo] }))}
                    className={`flex h-8 w-8 items-center justify-center rounded-full border transition-colors ${dispensacao[campo]
                        ? "border-green-200 bg-green-100 text-green-600"
                        : "border-red-200 bg-red-100 text-red-600"
                      }`}
                  >
                    {dispensacao[campo] ? <Check className="h-4 w-4" /> : <X className="h-4 w-4" />}
                  </button>
                </div>
              ))}
            </div>
