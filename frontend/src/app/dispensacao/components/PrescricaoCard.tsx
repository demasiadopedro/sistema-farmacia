...
export function PrescricaoCard(props: PrescricaoCardProps) {
  const {
    pacientes, medicamentos, prescricao, setPrescricao,
    handlePrescricaoSubmit, buscaPaciente, setBuscaPaciente,
    buscaMedicamento, setBuscaMedicamento, usoContinuo, setUsoContinuo,
    viasAdministracao, dropdownAberto, setDropdownAberto,
    dropdownMedAberto, setDropdownMedAberto, dropdownPacRef, dropdownMedRef,
  } = props;
    ...
    <Card className="rounded-xl border border-gray-100">
      ...
      <CardContent className="pt-5">
        <form onSubmit={handlePrescricaoSubmit} className="flex flex-col gap-5">
                  ...
                  medicamentosFiltrados.map((med) => (
                    ...
                    >
                      ...
                      <p className="text-xs text-green-600 font-medium">
                        Estoque total: {med.quantidadeTotal} unidades
                      </p>
                    </li>
                  ))
                ) : (
          ...
          <Button
            type="submit"
            className="mt-2 bg-[#1976d2] hover:bg-[#1565c0] text-white rounded-lg h-10 text-base font-medium"
          >
